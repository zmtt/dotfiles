from perceptual import hex_lr, lch, solve, to_linear
...
# renders as #1e1d19, exactly the gamma-1.8 transfer.
def col(hx):
...
for name, V, S in built:
    with open(os.path.join(OUT, f"{name}.xccolortheme"), "wb") as fp:
        plistlib.dump(theme(V, S), fp, sort_keys=True)
