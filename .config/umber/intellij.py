...
EMBER = 48.0  # the cursor hue; search borrows it so found text glows, not shouts

def surfaces(V):
    bg = V["background"]; dark = lum(bg) < 0.18
    bglr = lch(bg)[0]; nh = lch(V["foreground"])[2]
    d = 1 if dark else -1
    hue = {k: lch(V[s])[2] for k, s in (("red", "1"), ("green", "2"), ("yellow", "3"))}
    return {"line": hex_lr(bglr + d*0.045, 0.017, nh)[0],
            "panel": hex_lr(bglr + d*0.075, 0.017, nh)[0],
...
def studio_colors_dir():
    def ver(d):
        try: return tuple(int(x) for x in d.rsplit("AndroidStudio", 1)[1].split("."))
        except ValueError: return None  # AndroidStudioPreview* and the like
    dirs = [d for d in glob.glob(os.path.expanduser(
        "~/Library/Application Support/Google/AndroidStudio*")) if ver(d)]
    return os.path.join(max(dirs, key=ver), "colors")
...
if fail: raise SystemExit("contrast floor violated — not shipping")

OUT = studio_colors_dir()
os.makedirs(OUT, exist_ok=True)
for name, parent, V, S in built:
    open(os.path.join(OUT, f"{name}.icls"), "w").write(render(name, parent, V, S))
