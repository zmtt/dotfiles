...
def surfaces(V):
    ...
    d = 1 if dark else -1
    def n(off, C=0.017): return hex_lr(bglr + d*off, C, nh)[0]
    hue = {k: lch(V[s])[2] for k, s in (("red", "1"), ("green", "2"), ("yellow", "3"))}
    return {"line": n(0.045), "panel": n(0.075), "over": n(0.115, 0.019),
...
for name, background, V, S in built:
    open(os.path.join(COLORS, f"{name}.lua"), "w").write(render(name, background, V, S))
    open(os.path.join(THEMES, f"{name}.lua"), "w").write(render_lualine(name, V, S))
